...
function draw() {
  ...
  for (let iY = 0; iY < rows; iY++) {
      for (let iX = 0; iX < cols; iX++) {
        ...
        let lineWidth = (tileWidth + random(-tileWidth, tileWidth * 3)) * sin(millis() / 300) * tan(millis() / 300) * cos (millis() / 150);

        // apply different effects for randomly chosen lines
        let flipped = random(0, 1);
        if (flipped <= 0.5) {
          strokeWeight(map(mouseX, 0, width, 0, 30));
          line(x - sin(angle) * lineWidth / 2, y - cos(angle) * lineWidth / 2, x + sin(angle) * lineWidth / 2, y + cos(angle) * lineWidth / 2);
        }else{
          strokeWeight(map(mouseY, 0, height, 0, 30));
          line(x - cos(angle) * lineWidth / 2, y - sin(angle) * lineWidth / 2, x + cos(angle) * lineWidth / 2, y + sin(angle) * lineWidth / 2);
        }
      }
